...
sensible alternative to using the base-R
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) function to
produce diagnostic plots for model objects. Currently, `gglm` supports
all model objects that are supported by
...
For example, objects that are outputted from
[`stats::lm()`](https://rdrr.io/r/stats/lm.html),
[`lme4::lmer()`](https://rdrr.io/pkg/lme4/man/lmer.html), `brms::brm()`,
and many other common modeling functions will work with `gglm`. The
function
[`gglm::list_model_classes()`](https://graysonwhite.com/gglm/reference/list_model_classes.md)
...
[`gglm()`](https://graysonwhite.com/gglm/reference/gglm.md) function is
...
`lm` type object. Second, the `stat_*()` functions are used to produce
diagnostic plots by creating `ggplot2` layers. These layers allow for
plotting of particular model diagnostic plots within the `ggplot2`
framework.
...
weight. We can fit this model with
[`lm()`](https://rdrr.io/r/stats/lm.html), and then diagnose it easily
...
![](reference/figures/unnamed-chunk-5-2.png)
...
example of one of these functions is the
[`stat_fitted_resid()`](https://graysonwhite.com/gglm/reference/stat_fitted_resid.md)
function. With this function, we can take a closer look at just the
...
[`gglm()`](https://graysonwhite.com/gglm/reference/gglm.md) plots the
four default diagnostic plots when supplied a model object (this is
similar to `plot.lm()` in the case of an object generated by
[`lm()`](https://rdrr.io/r/stats/lm.html)). Note that can
[`gglm()`](https://graysonwhite.com/gglm/reference/gglm.md) take many
types of model object classes as its input, and possible model object
classes can be seen with
...
all are `ggplot2` layers used to create individual diagnostic plots. To
use these, follow Example 2.
